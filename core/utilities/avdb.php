...
class avdb {
    ...
    private function __refreshID($table) {
        if ($this->__checkTableExists($table)) {
            $s1 = 'ALTER TABLE ' . $table . ' DROP `id`';
            $s2 = 'ALTER TABLE ' . $table . ' AUTO_INCREMENT = 1';
            $s3 = 'ALTER TABLE ' . $table . ' ADD `id` int UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY FIRST';

            $s = $this->conn->prepare($s1);
            $s->execute();

            $s = $this->conn->prepare($s2);
            $s->execute();

            $s = $this->conn->prepare($s3);
            $s->execute();
    ...
    public function updateData($table, $fields, $values, $whereField, $whereValue)
    {        
        $this->__update($table, $fields, $values, $whereField, $whereValue);
    }
    
    public function selectData($table, $columns = null, $where = null, $logical = 'AND')
    {
        return $this->__select($table, $columns, $where, $logical);
    }
    
    public function deleteData($table, $where = null, $logical = 'AND')
    {
        return $this->__delete($table, $where, $logical);
    }

    public function refreshIdExample($table)
    {        
        $this->__refreshID($table);
    }
}